...
public:
	...
	struct Tex
	{
		float t[2];
		Tex(){}
		Tex(const float &t0, const float &t1)
			{ t[0] = t0; t[1] = t1; }
		Tex(const float *v_)
			{ t[0] = v_[0]; t[1] = v_[1];  }
		template <class S> explicit Tex(const S &x)
			{ t[0] = x[0];  t[1] = x[1];  }
		float &operator[] (int i) { return t[i]; }
		const float &operator[] (int i) const { return t[i]; }
		operator const float * () const { return &(t[0]); }
		operator const float * () { return &(t[0]); }
		operator float * () { return &(t[0]); }		
	};

	struct TexStruct
	{
		::std::vector<Tex> tCoords;
...
public:
	static void  readMTL(const ::std::string &filename,TriMesh *mesh = 0);
	static TriMesh *readOBJ(const char *filename);
