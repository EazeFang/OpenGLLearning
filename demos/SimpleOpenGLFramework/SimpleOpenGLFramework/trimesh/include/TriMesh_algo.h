...
namespace trimesh {
extern void combine(TriMesh *mesh, const TriMesh &mesh_add);
extern std::vector<TriMesh> split(TriMesh *mesh,bool isApart);
...
extern bool intersectTriangle(const point& orig, const point& dir,
		point& v0, point& v1, point& v2,
		float* t, float* u, float* v);
extern void intersectMesh(TriMesh *mesh, const point& orig, const point& dir,std::vector<int> &inter_faces_indices, std::vector<point> &intersect_pts,std::vector<float> &t_lists );

// Smooth mesh by Laplace method
extern void laplace_smooth(TriMesh *mesh,float alpha = 1.0f);
